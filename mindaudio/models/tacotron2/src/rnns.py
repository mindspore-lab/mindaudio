...
class DynamicRNN(nn.Cell):
    ...
    def __init__(self, mode):
        ...
        self.cell = rnncell_type_table.get(mode, None)
    ...
    def variable_recurrent(self, x, h, seq_length, w_ih, w_hh, b_ih, b_hh):
        ...
        seq_length = ops.BroadcastTo((hidden_size, -1))(seq_length)
...
class RNNBase(nn.Cell):
    ...
    def _stacked_bi_dynamic_rnn(self, x, h, seq_length, weights):
        ...
        for i in range(self.num_layers):
                ...
                h_b_i = (
                    self.expand_dims(
                        h_b_i[0], 0), self.expand_dims(
                            h_b_i[1], 0))
            if seq_length is None:
                x_b = ops.ReverseV2([0])(pre_layer)
            else:
                x_b = ops.ReverseSequence(0, 1)(pre_layer, seq_length)
            ...
            if seq_length is None:
                output_b = ops.ReverseV2([0])(output_b)
            else:
                output_b = ops.ReverseSequence(0, 1)(output_b, seq_length)
