...
class ASRModelWithAcc(nn.Cell):
    ...
    def construct(self,
        ...
        encoder_out_lens = self.cast(
            encoder_mask.squeeze((1, )).sum(axis=1),
            mstype.int32,
        )
    ...
    def _calc_att_loss(self, encoder_out: mindspore.Tensor, encoder_mask: mindspore.Tensor, ys_in_pad: mindspore.Tensor,
                       ys_out_pad: mindspore.Tensor, ys_masks: mindspore.Tensor, ys_sub_masks: mindspore.Tensor):
...
class ASRModel(nn.Cell):
    ...
    def construct(self,
        ...
        Returns:
            tuple: tensor of loss, tensor of attention accuracy
        """
        loss, _ = self.acc_net(xs_pad, ys_pad, ys_in_pad, ys_out_pad, xs_masks, ys_sub_masks, ys_masks, ys_lengths,
                               xs_chunk_masks)
        return loss
