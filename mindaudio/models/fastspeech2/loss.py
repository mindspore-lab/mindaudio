...
class FastSpeech2Loss(nn.Cell):
    ...
    def construct(self, items):
        ...
        src_masks = ~src_masks
        mel_masks = ~mel_masks
        log_duration_targets = ms.ops.log(duration_targets + ms.Tensor(1., dtype=dtype))
        ...
        pitch_loss = self.mse_loss(pitch_predictions, pitch_targets) / self.cast(mel_masks, dtype).mean()
        ...
        energy_loss = self.mse_loss(energy_predictions, energy_targets) / self.cast(mel_masks, dtype).mean()

        log_duration_predictions = log_duration_predictions * src_masks
        log_duration_targets = log_duration_targets * src_masks
        duration_loss = self.mae_loss(log_duration_predictions, log_duration_targets) / self.cast(src_masks, dtype).mean()

        mel_masks = mel_masks.expand_dims(-1)
        mel_predictions = mel_predictions * mel_masks
        mel_targets = mel_targets * mel_masks
        mel_loss = self.mae_loss(mel_predictions, mel_targets) / self.cast(mel_masks, dtype).mean()
