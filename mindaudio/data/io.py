...
class PaddedBatch:
    ...
    Then, by default, all np.ndarray valued elements get padded and support
    ...
    padded_keys : list, None
        (Optional) List of keys to pad on. If None, pad all np.ndarray
    device_prep_keys : list, None
        (Optional) Only these keys participate in collective memory pinning and moving with
        to().
        If None, defaults to all items with np.ndarray values.
    padding_func : callable, optional
        Called with a list of arrays to be padded together. Needs to return
        two arrays: the padded data, and another array for the data lengths.
    padding_kwargs : dict
    ...
    -------
    >>> batch = PaddedBatch([
    ...     {"id": "ex1", "foo": np.array([1.])},
    ...     {"id": "ex2", "foo": np.array([2., 1.])}])
    >>> # Attribute or key-based access:
    >>> batch.id
    ['ex1', 'ex2']
    >>> batch["id"]
    ['ex1', 'ex2']
    >>> # np.arrays get padded
    >>> type(batch.foo)
    <class '...PaddedData'>
    >>> batch.foo.data
    array([[1., 0.],
    ...
    >>> _ = batch.to(dtype=np.half)
    >>> batch.foo.data
    array([[1., 0.],
            [2., 1.]], dtype=np.float16)
    >>> batch.foo.lengths
    array([0.5000, 1.0000], dtype=np.float16)

    >>> # Basic stacking collation deals with non padded data:
    >>> batch = PaddedBatch([
    ...     {"spk_id": np.array([1]), "wav": np.array([.1, .0, .3])},
    ...     {"spk_id": np.array([2]), "wav": np.array([.2, .3, -.1])}],
    ...     padded_keys=["wav"])
    >>> batch.spk_id
    array([[1],
            [2]])
    ...
    def __init__(
            ...
            examples,
            padded_keys=None,
            device_prep_keys=None,  # 跨平台 CUDA、CPU
        ...
        for key in self.__keys:
            ...
            if (padded_keys is not None and key in padded_keys) or (
                    padded_keys is None and isinstance(values[0], np.ndarray)
            ):
            ...
            if (device_prep_keys is not None and key in device_prep_keys) or (
                    device_prep_keys is None and isinstance(values[0], np.ndarray)
            ):
    ...
    def __iter__(self):
        ...
        -------
        >>> batch = PaddedBatch([
        ...     {"id": "ex1", "val": np.array([1.])},
        ...     {"id": "ex2", "val": np.array([2., 1.])}])
        >>> ids, vals = batch
        >>> ids
        ['ex1', 'ex2']
        """
