...
class DynamicLossScaleUpdateCell(Cell):
    ...
    def __init__(self, loss_scale_value, scale_factor, scale_window):
        ...
        self.cur_iter = Parameter(Tensor(1, dtype=mstype.int32),
                                  name="current_iterator_step")
        self.last_overflow_iter = Parameter(Tensor(0, dtype=mstype.int32),
    ...
    def construct(self, loss_scale, overflow):
        ...
        loss_scale_on_overflow = self.select(
            overflow_cond,
            self.max(loss_scale * self.reciprocal(self.scale_factor),
                     self.minimum_loss_scale), loss_scale)
        should_inc = self.less_equal(self.scale_window,
                                     self.cur_iter - self.last_overflow_iter)
        last_iter_cond = self.logic_or(overflow_cond, should_inc)
        last_overflow_iter = self.select(last_iter_cond, self.cur_iter,
                                         self.last_overflow_iter)
        last_iter = F.assign(self.last_overflow_iter, last_overflow_iter)
        update_scale_cond = self.logic_and(should_inc,
                                           self.logic_not(overflow_cond))
        scale_mul_res = loss_scale_on_overflow * self.scale_factor
        scaled_loss_scale = self.select(update_scale_cond, scale_mul_res,
                                        loss_scale_on_overflow)
...
class TrainOneStepWithLossScaleCell(TrainOneStepCell):
    ...
    def construct(self, *inputs):
        ...
        return loss, cond, scaling_sens, overflow
    ...
    def get_overflow_status(self, status, compute_output):
        ...
        if not self.gpu_target:
            ...
            flag_sum = self.hyper_map(F.partial(_grad_overflow),
                                      compute_output)
