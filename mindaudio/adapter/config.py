...
def parse_cli_to_yaml(parser,
    ...
    for item in cfg:
        if not isinstance(cfg[item], list) and not isinstance(cfg[item], dict):
            help_description = (helper[item] if item in helper else
                                "Please reference to {}".format(cfg_path))
...
def get_config(config=""):
    ...
    if config == "":
        raise ValueError(
            "config name should be choose in ['wav2vec_pretrain', 'asr_config']"
        )
    if config == "wav2vec_pretrain":
        yaml_file = "../../config/wav2vec/" + config + ".yaml"
    else:
