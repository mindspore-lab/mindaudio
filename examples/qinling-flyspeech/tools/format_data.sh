...
if [ -n "${bpecode}" ]; then
    if [ "${trans_type}" == "cn_char_en_bpe" ]; then
        tools/text2token.py -s 1 -n 1 -m ${bpecode} ${dir}/text \
            --trans_type ${trans_type} > ${tmpdir}/output/token.scp
    else
    ...
    tools/text2token.py -s 1 -n 1 -l ${nlsyms} ${dir}/text \
        --trans_type ${trans_type} > ${tmpdir}/output/token.scp
elif [ -n "${raw}" ]; then
    cat $dir/text > ${tmpdir}/output/token.scp
else
    tools/text2token.py -s 1 -n 1 ${dir}/text \
        --trans_type ${trans_type} > ${tmpdir}/output/token.scp
fi
