...
from flyspeech.utils.callback import (
    CalRunTimeCallback,
    EvalCallback,
...
def modelarts_func(callback_list, model_dir, steps_size, graph_dir, config_obj):
    ...
    callback_list.append(
        SyncDirEndCallback(
            local_path=model_dir, obs_path=obs_model_dir, delete_old=True
        )
    )
    if config_obj.save_graphs:
        obs_graphs_dir = os.path.join(config_obj.train_url, "graph")
        callback_list.append(
            SyncDirEndCallback(local_path=graph_dir, obs_path=obs_graphs_dir)
        )
    if "MS_COMPILER_CACHE_PATH" in os.environ and not config_obj.compile_url:
        ...
        callback_list.append(
            SyncDirEndCallback(
                local_path=os.environ["MS_COMPILER_CACHE_PATH"], obs_path=cache_dir
            )
        )
