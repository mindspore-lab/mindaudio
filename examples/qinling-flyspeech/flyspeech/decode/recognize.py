...
from flyspeech.utils.mask import subsequent_mask, make_pad_mask
    ...
    if full_graph:
        ...
        for i in range(1, maxlen + 1):
            hyps_sub_masks.append(
                np.pad(subsequent_mask(i), ((0, maxlen - i), (0, maxlen - i)),
                       'constant',
        ...
        input_ids, best_hyps_index, best_scores = model.predict(xs_pad, xs_masks, xs_lengths, hyps_sub_masks,
                                                                input_ids_tensor, scores, end_flag, base_index)
        ...
        if pretrained_model:
            xs_pad, xs_masks = \
            model.predict_network.backbone.acc_net.pretrained_model.acc_net.extrator_feature_only(xs_pad, xs_masks,
                                                                                                  xs_lengths)
            if model.predict_network.backbone.acc_net.feature_post_proj:
                xs_pad = model.predict_network.backbone.acc_net.feature_post_proj(xs_pad)
        xs_masks = xs_masks[:, :, :-2:2][:, :, :-2:2]
        encoder_out, encoder_mask = model.predict_network.backbone.acc_net.encoder(
            xs_pad, xs_masks, xs_masks)  # (1, 124, 256)  (1, 1, 124)
        ...
        encoder_out_np = np.tile(np.expand_dims(encoder_out.asnumpy(), 1), (1, beam_size, 1, 1))  # (1, 10, 124, 256)
        encoder_out_np = encoder_out_np.reshape((running_size, maxlen, encoder_dim))  # (10, 124, 256)
        # (B*N, 1, max_len)
        encoder_mask_np = np.tile(np.expand_dims(encoder_mask.asnumpy(), 1), (1, beam_size, 1, 1))
        encoder_mask_np = encoder_mask_np.reshape((running_size, 1, maxlen))  # (10, 1, 124)
        ...
        for i in range(1, maxlen + 1):
            ...
            input_ids, valid_length, end_flag, scores, best_k_pred = model.predict(Tensor(encoder_out_np),
                                                                                   Tensor(encoder_mask_np),
                                                                                   Tensor(input_ids), Tensor(hyps_mask),
                                                                                   Tensor(valid_length),
                                                                                   Tensor(end_flag), scores,
                                                                                   Tensor(base_index))
            input_ids = input_ids.asnumpy()
...
def ctc_prefix_beam_search(model, xs_pad, xs_masks, beam_size, xs_lengths):
    ...
    assert batch_size == 1
    encoder_out, encoder_mask, top_k_logp_list, top_k_index_list = model.predict(xs_pad, xs_masks, xs_lengths)
...
def attention_rescoring(
    ...
    decoder_out = model_rescore.predict(encoder_out, encoder_mask, hyps_in_pad, hyps_sub_masks)
