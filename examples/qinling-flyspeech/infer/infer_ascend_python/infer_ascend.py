...
from infer.infer_ascend_python.recognize import (
    attention_rescoring,
    ctc_greedy_search,
    ctc_prefix_beam_search,
    recognize,
)
from infer.infer_ascend_python.utils import (
    get_padding_length,
    load_language_dict,
    make_pad_mask,
)
...
def run_infer(args_param):
    ...
    # load network
    if (
        config.decode_mode == "ctc_greedy_search"
        or config.decode_mode == "ctc_prefix_beam_search"
    ):
    ...
    # Ascend310 infer only supported not full_graph now for attention decode.
        ...
        encoder_model.build_from_file(
            encoder_model_path, mslite.ModelType.MINDIR_LITE, context
        )
        predict_model.build_from_file(
            predict_model_path, mslite.ModelType.MINDIR_LITE, context
        )
        ...
        rescore_model.build_from_file(
            rescore_model_path, mslite.ModelType.MINDIR_LITE, context
        )
    ...
    # load test data
    frame_bucket_limit = [
        int(i) for i in config["test_dataset_conf"]["frame_bucket_limit"].split(",")
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--infer_data_path", required=True, help="The folder path of the .wav file"
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--decode_mode", type=str, default="ctc_greedy_search", help="decode mode"
    )
